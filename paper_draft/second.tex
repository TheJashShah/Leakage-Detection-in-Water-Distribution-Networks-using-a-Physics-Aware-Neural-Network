...
\begin{abstract}
%% Text of abstract
Abstract example for the Results in Physics Journal. Here you provide a brief summary of the research and the results , this is a change.
\end{abstract}
...
\begin{figure}[H]
    ...
    \caption{Caption}
    \label{Distribution of Leak instances for all scenarios across nodes}
\end{figure}
...
\]
where $\bar{D}$ is the mean demand used for normalization.
...
\]
where $\bar{p}$ is the mean predicted pressure used for normalization.
...
\]
where $\lambda_{\mathrm{mass}}$ and $\lambda_{\mathrm{energy}}$ are both kept at $0.5$.
...
A major challenge in hydraulic localization is signal drift. When a leak occurs, the initial pressure drop is most severe at the source node. However, as the transient wave propagates, pressure drops accumulate at physical boundaries, such as dead-ends or downstream junctions, often causing purely statistical algorithms to drift away from the true source over time. To combat this, the proposed real-time module introduces an "Anti-Drift" mechanism combining time-decayed voting, graph-based neighbor validation, and a stability lock, triggered only when the ANN confidence exceeds a threshold ($\tau_{conf} > 0.75$).
...
\subsection{Detection Results and Detection}
...
To evaluate model stability under distributional shifts, we performed a two-dimensional sensitivity analysis over demand perturbation $\alpha \in [-0.15,\, 0.15]$ and pressure--demand coupling strength $\beta \in [0.2,\, 1.0]$. Figure~X and Figure~Y present the F1-score surfaces for the Physics-informed ANN and the baseline ANN, respectively.

Both models achieve comparable performance under nominal conditions ($\alpha = 0$). However, as the magnitude of perturbation increases, distinct degradation patterns emerge. In particular, under negative demand shifts combined with stronger pressure sensitivity (i.e., higher $\beta$), the baseline ANN exhibits rapid performance deterioration, whereas the Physics-ANN demonstrates more gradual degradation.

Figure~Z illustrates the performance gap $\Delta F_1 = F_{1,\text{Physics}} - F_{1,\text{ANN}}$. Positive regions indicate scenarios where physics-based constraints improve robustness. The advantage of the Physics-ANN becomes increasingly pronounced as $\beta$ increases, suggesting that embedding physical relationships enhances stability under stronger pressure demand coupling.

Importantly, both models converge under extreme positive demand shifts, indicating that severe distribution shifts affect data-driven and physics-informed models in a similar manner. This observation highlights that physics constraints improve robustness within realistic operating regimes rather than acting as a universal correction mechanism.
...
\end{axis}
\end{tikzpicture}
\caption{Worst-case robustness across demand perturbations. The Physics-informed ANN mitigates performance collapse under negative demand shifts.}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{Performance under extreme demand reduction ($\alpha=-0.15$). The Physics-ANN exhibits significantly slower degradation as pressure sensitivity increases.}
\end{figure}
