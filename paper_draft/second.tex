...
While the Deep Learning (ANN) module is responsible for the temporal detection of anomalies (identifying \textit{when} a leak occurs), the spatial localization (identifying \textit{where} the leak is located) is handled by a separate, physics-informed statistical engine. This hybrid approach leverages the strengths of both domains: the ANN captures complex, non-linear patterns to detect incipient events, while the localization module relies on hydraulic first principles—specifically, the analysis of pressure residuals relative to a dynamic baseline.

The localization logic operates on the hypothesis that a leak event introduces a localized pressure drop that is statistically significant compared to the node's historical variance. To ensure robustness against sensor noise and hydraulic transients, the system employs a "triggered snapshot" mechanism. Localization is not a continuous process; it is a conditional state activated only when the ANN confidence score exceeds a defined threshold ($\tau_{conf} > 0.75$). This ensures that the localization algorithm only processes frames where a high probability of a physical anomaly exists, thereby reducing false positives.

\subsection{Algorithm}
The localization algorithm proceeds in three distinct phases: Baseline Acquisition, Residual Standardization, and Temporal Aggregation.

\subsubsection{1. Dynamic Baseline Acquisition}
During normal operation (when the ANN probability $P(leak) < 0.2$), the system maintains a rolling buffer of pressure readings for all $N$ sensor nodes. Let $p_i(t)$ denote the pressure at node $i$ at time $t$. The system continuously updates the mean ($\mu_i$) and standard deviation ($\sigma_i$) for each sensor based on a window of stable steps $W_{stable}$.
Upon the detection of a leak event at time $t_{trigger}$, the updating of this buffer is immediately halted. This freezes the "healthy" state of the network, providing a static reference frame against which the unfolding leak event is compared.

\subsubsection{2. Residual Standardization (Z-Score Analysis)}
Once the system enters the active leak state, it calculates the raw pressure drop (residual) for every sensor at each time step $t$:
\begin{equation}
    \Delta p_i(t) = \mu_i - p_i(t)
\end{equation}
To account for sensor heterogeneity—where some nodes naturally exhibit higher variance than others—the raw drop is standardized into a Z-score. A minimum noise floor ($\epsilon \approx 0.1$) is applied to $\sigma_i$ to prevent hypersensitivity in stable sensors:
\begin{equation}
    Z_i(t) = \frac{\Delta p_i(t)}{\max(\sigma_i, \epsilon)}
\end{equation}
A physics-based gate is applied to filter out negligible fluctuations. A sensor $i$ is considered a candidate only if its drop exceeds a minimum hydraulic significance threshold ($\delta_{min} = 0.2$) and its statistical significance exceeds $3\sigma$:
\begin{equation}
    S_i(t) = 
    \begin{cases} 
    Z_i(t) & \text{if } \Delta p_i(t) > \delta_{min} \text{ and } Z_i(t) > 3.0 \\
    0 & \text{otherwise}
    \end{cases}
\end{equation}

\subsubsection{3. Cumulative Evidence and Localization}
To mitigate the impact of instantaneous sensor noise, the algorithm does not rely on a single time step. Instead, it employs a cumulative voting mechanism over the duration of the detected event ($t_{trigger}$ to $t_{current}$). The cumulative score $C_i$ for each node is updated recursively:
\begin{equation}
    C_i = \sum_{t=t_{trigger}}^{t_{current}} S_i(t)
\end{equation}
The predicted leak location $\hat{L}$ is identified as the node with the maximum accumulated evidence at the end of the event window:
\begin{equation}
    \hat{L} = \operatorname*{argmax}_{i \in \{1, \dots, N\}} (C_i)
\end{equation}
This accumulation ensures that the localization decision converges towards the node that exhibits the most consistent and statistically significant pressure drop over the duration of the anomaly.

\begin{algorithm}
\caption{Hybrid Leak Detection and Localization Strategy}
\label{alg:leak_localization}
\begin{algorithmic}[3]
\Require Real-time Pressure Data stream $P_t$, Trained ANN Model $\mathcal{M}$
\Ensure Detected Leak Event $E$, Localized Node $L_{est}$

\State \textbf{Initialize:} 
\State $\mathcal{B} \leftarrow \emptyset$ \Comment{Rolling Baseline Buffer}
\State $S \leftarrow \{0, \dots, 0\}$ \Comment{Cumulative Score Vector for $N$ nodes}
\State $LeakActive \leftarrow \text{False}$

\While{stream is active at time $t$}
    \State $x_t \leftarrow \text{Preprocess}(P_{t-w:t})$ \Comment{Sliding Window Input}
    \State $p_{leak} \leftarrow \mathcal{M}(x_t)$ \Comment{Get Probability from ANN}
    
    \If{$p_{leak} < 0.2$ \textbf{and not} $LeakActive$}
        \State $\text{Update}(\mathcal{B}, P_t)$ \Comment{Update healthy baseline stats}
        \State $\mu, \sigma \leftarrow \text{CalculateStats}(\mathcal{B})$
    \ElsIf{$p_{leak} > 0.75$}
        \State $LeakActive \leftarrow \text{True}$ \Comment{Trigger Event}
    \EndIf
    
    \If{$LeakActive$}
        \For{each sensor node $i \in \{1, \dots, N\}$}
            \State $\Delta_i \leftarrow \mu_i - P_{i,t}$ \Comment{Calculate Pressure Drop}
            \State $Z_i \leftarrow \Delta_i / \sigma_i$ \Comment{Standardize (Z-Score)}
            
            \State \emph{// Physics Gate: Ignore negligible drops}
            \If{$\Delta_i > \delta_{min}$ \textbf{and} $Z_i > 3.0$}
                \State $S_i \leftarrow S_i + Z_i$ \Comment{Accumulate Evidence}
            \EndIf
        \EndFor
        
        \State $L_{est} \leftarrow \operatorname*{argmax}_{i}(S)$ \Comment{Identify leading suspect}
    \EndIf
    
    \If{$p_{leak} < 0.50$ \textbf{for} $k$ steps}
        \State $LeakActive \leftarrow \text{False}$ \Comment{Reset System}
        \State \Return $L_{est}$
    \EndIf
\EndWhile
\end{algorithmic}
